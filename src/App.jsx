...
const parseDateKeyLocal = (dateKey) => {
  // Avoid timezone issues by constructing a local date at midnight.
  ...
  useEffect(() => {
    if (active) {
      const newParticles = Array.from({ length: 50 }, (_, i) => ({
        ...
        color: ['#fbbf24', '#34d399', '#60a5fa', '#f472b6', '#a78bfa'][
    ...
    <div className="modal-overlay" onClick={() => setIsOpen(false)}>
      <div className="search-modal" onClick={(e) => e.stopPropagation()}>
        <div className="search-header">
          ...
          />
          <button onClick={() => setIsOpen(false)} className="search-close">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="search-results">
          ...
          {results.map(([date, entry]) => {
            ...
            return (
              <div key={date} className="search-result-item">
                <div className="search-result-header">
                  <span className="search-result-date">{formatDate(date)}</span>
                  {moodData && (
                    <span
                      className="search-result-mood"
      ...
      <div className="stats-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {topMoodData && (
          <div className="mood-summary">
            ...
            <div
              className="top-mood"
              style={{
                background: `${topMoodData.color}20`,
                color: topMoodData.color,
              }}
            >
        ...
        <div className="stats-chart">
          ...
          <div className="chart-bars">
            {Object.entries(stats.monthlyEntries)
              .slice(-6)
              .map(([month, count]) => {
...
const AuthScreen = ({ onAuthComplete }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === 'signup') {
      ...
      users[trimmedUsername] = { password, createdAt: Date.now() };
      saveUsers(users);
      setError('');
    ...
    <div className="auth-wrapper">
      <div className="auth-card glass">
        ...
        <p className="auth-note">
          Front‑end only: data is stored in this browser&apos;s local storage per username.
        </p>
      </div>
    </div>
  );
    ...
    <header className="header">
      <div className="header-top">
        <div className="header-actions left">
          ...
          <button className="icon-btn" onClick={() => setStatsOpen(true)} aria-label="Statistics">
            <BarChart3 size={20} />
          </button>
        </div>
...
const EntryInput = ({ entries, setEntries, showConfetti, currentUser }) => {
  ...
  const handleSubmit = async () => {
    if (text.trim() && text.length <= MAX_CHARS) {
      ...
      saveEntries(currentUser, newEntries);
      setText('');
      setMood(null);
      setIsSaving(false);
...
const CalendarView = ({ entries }) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    // FIXED: compare local-midnight dates to avoid timezone edge cases.
    ...
    days.push(
      <button
        key={day}
        onClick={() => !isFuture && setSelectedDate(isSelected ? null : dateKey)}
  ...
  const entriesThisMonth = Object.keys(entries).filter((key) =>
        ...
        <div className={`entry-preview glass ${selectedDate ? 'visible' : ''}`}>
          {selectedDate && (
            <>
              ...
              {selectedText ? (
                <p className="preview-text">{selectedText}</p>
              ) : (
                <p className="preview-empty">No entry for this day</p>
              )}
            </>
          )}
        </div>
                  ...
                  return (
                    ...
                    >
                      <div className="entry-card-left">
                        ...
                        {moodData && (
                          <div
                            className="entry-mood-indicator"
    ...
    >
      <style>{`
        ...
        :root {
          ...
          --accent-light: #e8dcc8;
          --accent-dark: #a8895c;
          --accent-gradient: linear-gradient(135deg, #c4a574, #a8895c);
        }
        ...
        * {
        ...
        .sun-icon { color: #f59e0b; opacity: 0.5; }
        .moon-icon { color: #818cf8; opacity: 0.5; }
        ...
        .app-wrapper.dark .prompt-container {
          background: linear-gradient(135deg, rgba(196, 165, 116, 0.2), var(--bg-secondary));
        }
        ...
        .app-wrapper.dark .entry-textarea:focus {
        ...
        .save-button.ready:hover {
          transform: translateY(-3px);
          box-shadow: 0 8px 30px rgba(196, 165, 116, 0.5);
        }
        ...
        .success-glow {
          ...
          right: 0;
          height: 4px;
          background: linear-gradient(90deg, #4ade80, var(--accent), #4ade80);
        ...
        .app-wrapper.dark .calendar-day.today {
        ...
        .app-wrapper.dark .calendar-day.today .day-number {
          color: var(--accent);
        }
        ...
        .calendar-day.selected {
          ...
          box-shadow: 0 4px 15px rgba(196, 165, 116, 0.4);
        }
        ...
        .app-wrapper.dark .entry-date-badge {
          background: rgba(196, 165, 116, 0.2);
        }
      ...
      {currentUser ? (
        <>
          <SearchModal entries={entries} isOpen={searchOpen} setIsOpen={setSearchOpen} />
          ...
          {currentView === 'list' && <ListView entries={entries} />}
        </>
      ) : (
